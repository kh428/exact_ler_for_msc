"""Physical fault-weight endpoint with the audited SOFT geometry unchanged.

The source factor is 1 for no fault and x/nu per nonidentity outcome,
including outcomes whose response label is zero. All pending convolutions
and equality tests retain degrees zero through K. This is a separate
engine: the fixed-probability exact/float sources are not modified.
"""
...
from .truncated_polynomial import PolynomialNetwork


class PolynomialSourceHybrid(HybridFaultNetwork):
    ...
    def __init__(self, qubits, ring):
        self.network = PolynomialNetwork(ring)
    ...
    def source_channel(self, item, p):
        ...
        if item['kind'] == 'depol2':
            ...
            return
        if item['kind'] == 'depol1':
            mixed = np.array([[1+p/3, 1-p/3], [2*p/3, 0]], dtype=object)
        else:
            axis = 'Z' if item['source_kind'] == 'X_READOUT_FLIP' else item['source_kind'][0]
            if axis == 'X':
                mixed = np.array([[1, 1], [p, p]], dtype=object)
            elif axis == 'Z':
                mixed = np.array([[1+p, 1-p], [0, 0]], dtype=object)
            elif axis == 'Y':
                mixed = np.array([[1, 1], [p, -p]], dtype=object)
            else:
...
def polynomial_open_hybrid(model, ring, component=0):
    p = ring.x  # local name follows the audited geometry; this is x=p_phys/(1-p_phys)
    if component not in (0, 1):
        raise ValueError('Invalid polynomial component')
    ...
    builder = PolynomialSourceHybrid(sorted(data+ancillas), ring)
    net = builder.network
    ...
    net.source_metadata = {'variable': 'x=p/(1-p)', 'degree': ring.degree, 'prime': ring.prime, 'component': component,
                           'source_channels': len(model['channels']), 'events': builder.events,
                           'ancilla_normalization': str(Fraction(1, 1 << len(ancillas))),
                           'coefficient_arithmetic': 'exact F_q[x]/x^(K+1); one (1-p) stripped per retained source channel'}
    return net
